...
////////////////////////////////////////////////////////////////
//Converts object type to string and writes to file
////////////////////////////////////////////////////////////////
char *SerializeSchema(Schema *sch)
{    
		int memoryNo = AssignRecodMemory(sch),i=10;
		char *output,rand[10];
		output = (char *)malloc(3*memoryNo);
		//sending the data to status structure variable
...
{
			char rand[10],*outcome;
			outcome = (char *)malloc(10);
			SM_PageHandle ph;
			SM_FileHandle fh;
						...
						for(i = i + 1; i < strlen(ph) ; i++)
						{
							...
							if(strcmp(rand," ") == 0)
							{
								if(k == 1)
								{
									...
									strcpy(attrNames[j], outcome);
								}
								else if(k == 2)
								{
									dataTypes[j] = atoi(outcome);
								}
								k++;
								free(outcome);
								outcome = (char *)malloc(10);
...
extern RC createTable (char *name, Schema *schema)
{
		...
		if(status == RC_OK)
		{
			...
			if(status == RC_OK)
			{
				schInfo = SerializeSchema(schema);
				status=writeBlock(0, &fh, schInfo) ;
				if(status == RC_OK)
				{
					schInfo = NULL;
					free(schInfo);
...
extern int getNumTuples (RM_TableData *rel)
{
		...
		recordid.slot = 0;
		int status,c = 0;
		while(recordid.page < ((BM_BufferPoolInfo *)(record->bm)->mgmtData)->fh->totalNumPages && recordid.page > 0)// compare condition till the end of the table
		{
			if(getRecord (rel, recordid, rec) == RC_OK)
			{ 
				recordid.slot= 0;
				c++; //incerement tuple counter
				(recordid.page)++; //increment page id
			}
		}
		rec = NULL;
		free(rec);
		return c;// return the tuple counter
}


////////////////////////////////////////////////////////////////
//Returns the memory needed by schema
////////////////////////////////////////////////////////////////
int AssignRecodMemory(Schema *sch)
{
			int i, mem = 0;
			//add memory as necessary
			for(i = 0; i < sch->numAttr; i++)
			{
...
////////////////////////////////////////////////////////////////
//This function returns the next tuple that fulfils the given 
//scan condition
////////////////////////////////////////////////////////////////
RC next (RM_ScanHandle *scan, Record *record)
{
		...
		if(spool->condition == NULL)
		{
			while(recordid.page > 0 && recordid.page < ((BM_BufferPoolInfo *)(((RM_RecordPoolInfo *)(scan->rel)->mgmtData)->bm)->mgmtData)->fh->totalNumPages)
			{
				...
				if(status == RC_OK) //copy data if true
				{
...
{
		RM_RecordPoolInfo *rec=(RM_RecordPoolInfo *)rel->mgmtData;
		//compare primary key with hash map and insert key if not found
		...
		rec->emptyPages[0] = recordid.page; // setting the page number
...
extern RC deleteRecord (RM_TableData *rel, RID id)
{
		...
		if(id.page > 0 && id.page <=  ((BM_BufferPoolInfo *)(rec->bm)->mgmtData)->fh->totalNumPages)
		{
			...
			if(status == RC_OK)
			{
				...
				int status = getRecord (rel, recordid, r); //fetch record from the table
				if(status == RC_OK)
				{   
...
////////////////////////////////////////////////////////////////
//Free the memory used by records
////////////////////////////////////////////////////////////////
